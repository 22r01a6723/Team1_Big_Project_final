...
import com.Project1.Search.dto.SearchRequest;
import com.Project1.Search.model.Message;
import com.Project1.Search.repository.filter.SearchFilter;
import lombok.RequiredArgsConstructor;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.PageImpl;
import org.springframework.data.domain.Pageable;
...
public class MessageCustomRepository {
    ...
    public Page<Message> advancedSearch(SearchRequest request, Pageable pageable) {
        try {
            ...
            Criteria rootCriteria = new Criteria("tenantId.keyword").is(request.getTenantId());

            // Apply all available filters
            filters.forEach(filter -> filter.apply(request, rootCriteria));

            CriteriaQuery query = new CriteriaQuery(rootCriteria).setPageable(pageable);
            SearchHits<Message> searchHits = elasticsearchOperations.search(query, Message.class);
